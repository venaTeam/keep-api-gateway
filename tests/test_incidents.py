import importlib
import pytest
pytestmark = pytest.mark.skip(reason="Tests integration with worker/correlator")
...
try:
    from src.services.maintenance_windows_bl import MaintenanceWindowsBl
except ImportError:
...
from src.services.identity_manager.rbac import Admin
...
def test_incident_status_change_manual_alert_enrichment(
    ...
    with patch(
        "src.services.identity_manager.identity_managers.noauth.noauth_authverifier.NoAuthVerifier._verify_api_key",
...
def test_incident_bl_create_incident(db_session):
    ...
    with patch("src.services.incidents_bl.WorkflowManager", workflow_manager):
...
async def test_incident_bl_add_alert_to_incident(db_session, create_alert):
    ...
    with patch("src.services.incidents_bl.WorkflowManager", workflow_manager):
        with patch("src.services.incidents_bl.ElasticClient", elastic_client):
            incident_bl = IncidentBl(
                tenant_id=SINGLE_TENANT_UUID, session=db_session
            )
            incident_dto_in = IncidentDtoIn(
...
def test_incident_auto_resolve_only_if_active(db_session, create_alert):
    ...
    with patch(
        "src.services.event_management.process_event_task.IncidentBl.resolve_incident_if_require"
    ) as incident_bl_mock:
        create_alert(
            "alert-test",
            AlertStatus.RESOLVED,
...
def test_incident_not_created_maintenance(
    ...
    """
    if MaintenanceWindowsBl is None:
        pytest.skip("MaintenanceWindowsBl not available in keep-api-gateway")
    ...
    import src.services.maintenance_windows_bl
    importlib.reload(src.services.maintenance_windows_bl)
