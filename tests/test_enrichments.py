# test_enrichments.py
import pytest
pytestmark = pytest.mark.skip(reason="Tests integration with event-handler/worker")
import time
import uuid
...
try:
    from src.models.db.workflow import Workflow
except ImportError:
    Workflow = None
...
try:
    ...
    wait_for_workflow_execution = None
    wait_for_workflow_in_run_queue = None
...
def patch_get_tenants_configurations():
    ...
    try:
        with patch(
            "src.repositories.tenant_configuration.TenantConfiguration._TenantConfiguration.get_configuration",
            return_value=None,
        ):
            yield
    except Exception:
...
@pytest.mark.asyncio
async def test_run_extraction_rules_no_rules_applies(mock_session, mock_alert_dto):
    ...
    result_event = await enrichment_bl.run_extraction_rules(mock_alert_dto)
...
@pytest.mark.asyncio
async def test_run_extraction_rules_regex_named_groups(mock_session, mock_alert_dto):
    ...
    with patch("chevron.render", return_value="Test Alert"):
        enriched_event = await enrichment_bl.run_extraction_rules(mock_alert_dto)
...
@pytest.mark.asyncio
async def test_run_extraction_rules_event_is_dict(mock_session):
    ...
    with patch("chevron.render", return_value="Test Alert"):
        enriched_event = await enrichment_bl.run_extraction_rules(event)
...
@pytest.mark.asyncio
async def test_run_extraction_rules_no_rules(mock_session, mock_alert_dto):
    mock_session.query.return_value.filter.return_value.filter.return_value.order_by.return_value.all.return_value = []
...
@pytest.mark.asyncio
async def test_run_extraction_rules_attribute_no_template(mock_session, mock_alert_dto):
    rule = ExtractionRule(
        id=1,
        tenant_id="test_tenant",
        priority=1,
...
@pytest.mark.asyncio
async def test_run_extraction_rules_empty_attribute_value(mock_session, mock_alert_dto):
    rule = ExtractionRule(
        id=1,
        tenant_id="test_tenant",
        priority=1,
...
@pytest.mark.asyncio
async def test_run_extraction_rules_with_conditions(mock_session, mock_alert_dto):
    rule = ExtractionRule(
        id=2,
        tenant_id="test_tenant",
        priority=1,
...
@pytest.mark.asyncio
async def test_run_mapping_rules_applies(mock_session, mock_alert_dto):
...
@pytest.mark.asyncio
async def test_run_mapping_rules_with_regex_match(mock_session, mock_alert_dto):
    rule = MappingRule(
        id=1,
        tenant_id="test_tenant",
        priority=1,
...
@pytest.mark.asyncio
async def test_run_mapping_rules_no_match(mock_session, mock_alert_dto):
    rule = MappingRule(
        id=1,
        tenant_id="test_tenant",
        priority=1,
...
@pytest.mark.asyncio
async def test_check_matcher_with_and_condition(mock_session, mock_alert_dto):
...
@pytest.mark.asyncio
async def test_check_matcher_with_or_condition(mock_session, mock_alert_dto):
    # Setup a mapping rule with || condition in matchers
    rule = MappingRule(
        id=1,
        tenant_id="test_tenant",
...
@pytest.mark.asyncio
async def test_mapping_rule_with_elastic(mock_session, mock_alert_dto, setup_alerts):
    import os
...
@pytest.mark.asyncio
async def test_enrichment_with_elastic(
...
@pytest.mark.parametrize("test_app", ["NO_AUTH"], indirect=True)
@pytest.mark.asyncio
async def test_enrichment(db_session, client, test_app, mock_alert_dto):
    # add some rule
    rule = MappingRule(
        id=1,
        tenant_id=SINGLE_TENANT_UUID,
...
@pytest.mark.parametrize("test_app", ["NO_AUTH"], indirect=True)
@pytest.mark.asyncio
async def test_disposable_enrichment(db_session, client, test_app, mock_alert_dto):
...
@pytest.mark.asyncio
async def test_topology_mapping_rule_enrichment(mock_session, mock_alert_dto):
    ...
    with patch(
        "src.services.enrichments_bl.get_topology_data_by_dynamic_matcher",
        return_value=mock_topology_service,
    ):
        # Mock the enrichment database function so no actual DB actions occur
        with patch(
            "src.services.enrichments_bl.enrich_alert_db"
        ) as mock_enrich_alert_db:
            # Run the mapping rule logic for the topology
            result_event = await enrichment_bl.run_mapping_rules(mock_alert_dto)
...
@pytest.mark.asyncio
async def test_run_mapping_rules_with_complex_matchers(mock_session, mock_alert_dto):
...
@pytest.mark.asyncio
async def test_run_mapping_rules_enrichments_filtering(mock_session, mock_alert_dto):
...
def test_incident_workflow_enrichment_integration(db_session, client, test_app):
    ...
    """
    if Workflow is None:
        pytest.skip("Workflow model not available in keep-api-gateway")
