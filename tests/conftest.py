...
# Note: Workflow models and process_event don't exist in keep-api-gateway
...
def db_session(request, monkeypatch, tmp_path):
    ...
    session.commit()
    # Note: Workflow models don't exist in keep-api-gateway, skipping workflow prepopulation

    with patch("src.repositories.db.engine", mock_engine):
        with patch("src.repositories.db_utils.create_db_engine", return_value=mock_engine):
            with patch("src.repositories.alerts.engine", mock_engine):
                yield session
