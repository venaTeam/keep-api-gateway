...
bypassed the ``os.environ.get(..., default)`` default, ``os.makedirs("")``
raised and was silently swallowed, and prometheus_client wrote its mmap files
into the process CWD. These tests call the resolver directly rather than
reloading the module (a reload would re-register every collector against the
default registry).
"""
import os
