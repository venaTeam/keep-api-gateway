import logging
import pytest
pytestmark = pytest.mark.skip(reason="Tests require proper mock engine")
import os
import tempfile
...
def test_provision_provider_from_yaml(temp_providers_dir, sample_provider_yaml, caplog):
    ...
    with patch.dict(os.environ, {"KEEP_PROVIDERS_DIRECTORY": temp_providers_dir}):
        with (
            patch(
                "src.providers.providers_service.ProvidersService.is_provider_installed",
                return_value=False,
            ),
            patch(
                "src.providers.providers_service.ProvidersService.install_provider",
                return_value=mock_provider,
            ) as mock_install,
            patch(
                "src.providers.providers_service.provision_deduplication_rules"
            ) as mock_provision_rules,
            patch("src.repositories.db.get_all_provisioned_providers", return_value=[]),
            patch(
