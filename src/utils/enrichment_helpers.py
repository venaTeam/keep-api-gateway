...
from src.repositories.db import existed_or_new_session
...
def convert_db_alerts_to_dto_alerts(
    ...
    with existed_or_new_session(session) as session:
        # Resolve LastAlert rows: prefer the one already attached by the query
        # layer; batch-fetch the rest by (tenant_id, fingerprint).
        last_alerts_by_key = {}
        missing_keys = set()
        for _object in alerts:
            alert = _object if isinstance(_object, Alert) else _object[0]
            attached = getattr(alert, "_last_alert", None)
            if attached is not None:
                last_alerts_by_key[(alert.tenant_id, alert.fingerprint)] = attached
            else:
                missing_keys.add((alert.tenant_id, alert.fingerprint))

        if missing_keys:
            fingerprints = {fp for (_, fp) in missing_keys}
            tenant_ids = {tid for (tid, _) in missing_keys}
            rows = session.exec(
                select(LastAlert)
            ...
            for la in rows:
                last_alerts_by_key.setdefault((la.tenant_id, la.fingerprint), la)
        ...
        with tracer.start_as_current_span("alerts_enrichment"):
            for _object in alerts:
                ...
                # episode "started_at" marker (= LastAlert.first_timestamp) is set by
                # the query layer onto the Alert instance.
                started_at = getattr(alert, "_started_at", None)
                if started_at is not None:
                    alert_payload["started_at"] = started_at
